...
class HrUpdateProcess(models.Model):
    ...
    employee_id = fields.Many2one('hr.employee', string="Employee",domain="[('branch_id','=',branch_id)]")
    employee_ids = fields.Many2many('hr.employee', string="Employees",domain="[('branch_id','=',branch_id)]")
    update_type = fields.Selection([('salary', 'Salary'),
                                    ('department', 'Department'),
                                    ('job_position', 'Job Position')], string="Update Type")
    ...
    grade_id = fields.Many2one('hr.grade',string="Grade",domain="[('job_id','=',job_id),('branch_id','=',branch_id)]")
    level_id = fields.Many2one('hr.level',string="Level",domain="[('grade_id','=',grade_id)]")
    ...
    def update_employees(self, employees):
        for rec in employees:
            ...
            if self.job_id:
                ...
                rec.grade_id = self.grade_id.id
                rec.level_id = self.level_id.id

    def get_employees(self):
        if self.type == 'select_employees':
            return self.env['hr.employee'].search([('id', 'in', self.employee_ids.ids),('branch_id','=',branch_id.id)])
