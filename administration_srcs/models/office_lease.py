...
class OfficeLease(models.Model):
    ...
    first_partner_id = fields.Many2one('res.partner', string='First Party')
    total_amount_items = fields.Float(string="Total")

    @api.onchange('lease_details')
    def call_amount_items(self):
        sum = 0.0
        for rec in self.lease_details:
            sum += rec.cost
            self.total_amount_items = sum
