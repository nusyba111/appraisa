...
class HrMission(models.Model):
	...
	ticket_id = fields.Many2one('hr.mission.ticket',string="Ticket")
	fleet_request = fields.One2many('fleet.request','mission_id',string="Fleet Request")
	...
	def action_approve(self):
		...
		if self.travel_by == 'air':
			ticket = self.env['hr.mission.ticket'].create({
				'travel_agency' : self.air_company.id,
				'date_from' : self.date_from,
				'date_to':self.date_to,
				'account_id':self.mission_account_id.id,
				'doner':self.doner.id,
		...
		elif self.travel_by == 'car':
			for line in self.mission_line_ids:
				fleet = self.env['fleet.request'].create({
					'employee':line.employee_id.id,
					'department':line.department_id.id,
					'date_from':line.start_date,
					'date_to':line.end_date,
					'purpose': "Travel",
					'mission_id':line.mission_request_id.id,
					})
		
		for line in self.mission_line_ids:
			request_line = {
				'project_id':line.mission_request_id.project.id,
				'analytic_activity_id':line.mission_request_id.activity.id,
				'donor_id':line.mission_request_id.doner.id,
				'request_amount':line.total_amount,
			}
			line_list.append((0, 0, request_line))
		payment_request = self.env['payment.request'].create({
			'journal_id':self.journal_id.id,
			'reason':'Travel Allowance',
			'budget_line_ids':line_list
			})
		self.payment_request = payment_request.id	


			

	def open_tickect(self):
	    # pickings = self.mapped('picking_ids')
		action = {
		'name':_("Ticket"),
		'view_mode':'tree,form',
		'res_model':'hr.mission.ticket',
		'type':'ir.actions.act_window',
		'domain':[('id', '=',self.ticket_id.id)],
		}
		return action

	def open_fleet_request(self):
		action = {
		'name':_("Fleet Request"),
		'view_mode':'tree,form',
