...
class SalaryPlan(models.Model):
    ...
    def _compute_coverage_in_usd(self):
        for rec in self:
<<<<<<< HEAD
            rec.coverage_in_usd = (rec.contract_id.wage * rec.percentage_of_covering) / 100
=======
...
class HRPayslipHelal(models.Model):
    ...
    def _prepare_line_values(self, line, account_id, date, debit, credit):
        ...
        #         res['activity_id'] = plan.activity.id
        #         res['location_id'] = plan.location.id 
        #         res['partner_id'] = plan.doner_id.id 
        #         if debit > 0:
        #             res['debit'] = (plan.percentage_of_covering / 100) * debit
        #         if credit > 0:
        #             res['credit'] = (plan.percentage_of_covering / 100) * credit
        #         pay_line.append((0,0,res))
        #         print('fkkggggggggggggggggggggggggggggggggg',pay_line)
        #         print('____________________________________debit\n\n\n\n',res['debit'])
        #         print('___________________________________credit\n\n\n\n',res['credit'])
        # else:
                ...
                for slip in slip_mapped_data[journal_id][slip_date]:
                    ...
                    for line in slip.line_ids.filtered(lambda line: line.category_id):
                            ...
                            if not debit_line:
                                if line.slip_id.contract_id.salary_plan:
                                    for plan in line.slip_id.contract_id.salary_plan:
                                        ...
                                        debit_line['analytic_account_id'] = plan.covered_by.id
                                        debit_line['activity_id'] = plan.activity.id
                                        debit_line['location_id'] = plan.location.id 
                                        debit_line['partner_id'] = plan.doner_id.id 
                                        if debit > 0:
                                            debit_line['debit'] = (plan.percentage_of_covering / 100) * debit
                                        if credit > 0:
                                            debit_line['credit'] = (plan.percentage_of_covering / 100) * credit
                                        print('____________________________________debit\n\n\n\n',debit_line['debit'])
                                        print('___________________________________credit\n\n\n\n',debit_line['credit'])
                                        line_ids.append(debit_line)
                                        print('___________________________________debitline_ids\n\n\n\n',line_ids)
                            else:
                        ...
                        if credit_account_id: # If the rule has a credit account.
                            ...
                            #         for plan in line.slip_id.contract_id.salary_plan:
                            #             credit_line = self._prepare_line_values(line, credit_account_id, date, debit, credit)
                            #             credit_line['analytic_account_id'] = plan.covered_by.id
                            #             credit_line['activity_id'] = plan.activity.id
                            #             credit_line['location_id'] = plan.location.id 
                            #             credit_line['partner_id'] = plan.doner_id.id 
                            #             if debit > 0:
                            #                 debit_line['debit'] = (plan.percentage_of_covering / 100) * debit
                            #             if credit > 0:
                            #                 debit_line['credit'] = (plan.percentage_of_covering / 100) * credit
                            ...
                            #             print('___________________________________creditline_ids\n\n\n\n',line_ids)
                                line_ids.append(credit_line)
                            else:
